    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .schools-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:14px; }
        @media (max-width:900px) { .schools-grid { grid-template-columns:repeat(2,1fr); } }
        @media (max-width:560px) { .schools-grid { grid-template-columns:1fr; } }
        .school-table { width:100%; border-collapse:collapse; }
        .school-table th { padding:11px 16px; text-align:left; font-size:11px; font-weight:700; color:${C.textLight}; text-transform:uppercase; letter-spacing:0.07em; background:${C.bg}; font-family:'Inter',sans-serif; border-bottom:1.5px solid ${C.borderLight}; white-space:nowrap; }
        .school-table th.center { text-align:center; }
        .school-table td { padding:13px 16px; font-size:13px; color:${C.text}; font-family:'Inter',sans-serif; border-bottom:1px solid ${C.borderLight}; }
        .school-row:hover td { background:${C.bg}; }
        ...
        @media (max-width:540px) { .hide-sm { display:none !important; } .school-table th, .school-table td { padding:11px 10px; } }
      `}</style>
        ...
        <Panel>
          ...
          <div style={{ padding: "15px 20px", borderBottom: `1.5px solid ${C.borderLight}`, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12, background: `linear-gradient(90deg, ${C.bg} 0%, ${C.white} 100%)` }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
              <div style={{ width: 32, height: 32, borderRadius: 10, background: `${C.sky}22`, display: "flex", alignItems: "center", justifyContent: "center", border: `1.5px solid ${C.sky}33` }}>
                <Building2 size={15} color={C.slate} strokeWidth={2} />
              </div>
              <span style={{ fontFamily: "'Inter', sans-serif", fontSize: 13, fontWeight: 700, color: C.text }}>All Schools</span>
          ...
          {!loading && error && (
            <div style={{ margin: 20, padding: "14px 18px", borderRadius: 14, background: "#fef2f2", border: "1px solid #fecaca", color: "#dc2626", fontSize: 13, display: "flex", alignItems: "center", justifyContent: "space-between", fontFamily: "'Inter', sans-serif" }}>
              <span>{error}</span>
              <button onClick={fetchSchools} style={{ fontSize: 12, fontWeight: 700, color: "#dc2626", background: "none", border: "none", cursor: "pointer", textDecoration: "underline" }}>Retry</button>
            </div>
          )}
              ...
              <table className="school-table">
                <thead>
                  <tr>
                    ...
                    <th className="center hide-md">Email</th>
                ...
                <tbody>
                  {filtered.map((s, i) => (
                    ...
                    >
                      <td>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <div style={{ width: 34, height: 34, borderRadius: 10, flexShrink: 0, background: `linear-gradient(135deg, ${C.sky}22, ${C.mist}44)`, display: "flex", alignItems: "center", justifyContent: "center", border: `1.5px solid ${C.borderLight}` }}>
                            <Building2 size={14} color={C.deep} strokeWidth={1.8} />
                          </div>
                          <div>
                            <p style={{ margin: 0, fontWeight: 700, fontSize: 13, color: C.text }}>{s.name}</p>
                            <p style={{ margin: 0, fontSize: 10, color: C.textLight }}>{s.code}</p>
                          </div>
                        </div>
                      </td>
                      <td style={{ textAlign: "center", color: C.textMid, fontSize: 12 }}>{s.city || "—"}</td>
                      ...
                      <td className="hide-md" style={{ textAlign: "center", color: C.textLight, fontSize: 12 }}>{s.email || "—"}</td>
                      <td className="hide-md" style={{ textAlign: "center", color: C.textLight, fontSize: 12 }}>{s.phone || "—"}</td>
                      <td style={{ textAlign: "center" }}><StatusBadge isActive={s.isActive} /></td>
                      <td>
                      ...
                      <td colSpan="7">
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "48px 0", gap: 10 }}>
                          <div style={{ width: 50, height: 50, borderRadius: 16, background: `${C.sky}18`, display: "flex", alignItems: "center", justifyContent: "center", border: `1px solid ${C.sky}33` }}>
                            <Building2 size={22} color={C.sky} strokeWidth={1.5} />
                          </div>
