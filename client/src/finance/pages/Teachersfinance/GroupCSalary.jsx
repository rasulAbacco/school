...
export default function GroupCSalary() {
    ...
    const fetchStaff = async () => {
        try {
            const res = await fetch(`${API_URL}/api/groupc/salary/list/all`, { credentials: "include" });
    ...
    const handleSave = async () => {
        try {
            if (editData) {
                await fetch(`${API_URL}/api/groupc/salary/update/${editData.id}`, {
                    method: "PUT", credentials: "include",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(form),
                });
            } else {
                await fetch(`${API_URL}/api/groupc/salary/create`, {
    ...
    const handleDelete = async (id) => {
        ...
        await fetch(`${API_URL}/api/groupc/salary/delete/${id}`, { method: "DELETE", credentials: "include" });
        fetchStaff();
    };

    const handlePay = async (id) => {
        try {
            await fetch(`${API_URL}/api/groupc/salary/pay/${id}`, { method: "PATCH", credentials: "include" });
