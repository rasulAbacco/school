// src/parent/pages/chat/Components/ParentChatList.jsx
...
const ParentChatList = ({ selectedChat, onSelectChat, onChatCreated, studentId }) => {
  ...
  const fetchUsersByRole = async (role) => {
    try {
      ...
      if (role === "ADMIN") {
        res = await fetch(`${API_URL}/api/chat?role=ADMIN`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const data = await res.json();
        setAdmins(data.data || []);
      } else if (role === "TEACHER" && studentId) {
        res = await fetch(`${API_URL}/api/chat/parent-teachers?studentId=${studentId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const data = await res.json();
        ...
        <div className="bg-[#384959] p-4 flex items-center justify-between gap-2 flex-shrink-0">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <h2 className="text-white text-base font-semibold tracking-tight truncate">
              {selectionMode ? `${selectedIds.size} selected` : "Parent Chats"}
            </h2>
          </div>
