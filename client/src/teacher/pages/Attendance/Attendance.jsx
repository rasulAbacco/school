  ...
  return (
    <div style={{ position: "fixed", bottom: 24, right: 24, display: "flex", flexDirection: "column", gap: 8, zIndex: 9999, pointerEvents: "none" }}>
      {toasts.map((t) => {
        ...
        const Icon = cfg.icon;
        return (
          <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", borderRadius: 14, background: cfg.bg, border: `1px solid ${cfg.border}`, boxShadow: "0 4px 20px rgba(56,73,89,0.13)", minWidth: 260, maxWidth: 340, pointerEvents: "auto", fontFamily: "'Inter', sans-serif", opacity: t.leaving ? 0 : 1, transform: t.leaving ? "translateX(20px)" : "translateX(0)", transition: t.leaving ? "all 0.2s ease" : "none", animation: t.leaving ? "none" : "toastIn 0.28s cubic-bezier(0.34,1.56,0.64,1) both" }}>
            ...
            <button onClick={() => onRemove(t.id)} style={{ background: "none", border: "none", cursor: "pointer", color: cfg.color, opacity: 0.5, padding: 0, display: "flex", alignItems: "center" }}>
              <X size={13} />
            </button>
          </div>
        );
...
function StatCard({ label, value, icon: Icon, accent }) {
  return (
    <div style={{ background: C.white, borderRadius: 18, border: `1.5px solid ${C.borderLight}`, boxShadow: "0 2px 16px rgba(56,73,89,0.06)", overflow: "hidden", position: "relative" }}>
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, ${accent}, ${C.deep})`, borderRadius: "18px 18px 0 0" }} />
      <div style={{ padding: "16px 18px 14px" }}>
        <div style={{ width: 36, height: 36, borderRadius: 12, background: `${accent}22`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 10, border: `1px solid ${accent}33` }}>
          <Icon size={16} color={accent} strokeWidth={2} />
        </div>
        <p style={{ margin: 0, fontSize: 24, fontWeight: 800, color: C.text, lineHeight: 1, fontFamily: "'Inter', sans-serif" }}>{value}</p>
  ...
  return (
    <div style={{ width: "100%" }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
        <span style={{ fontFamily: "'Inter', sans-serif", fontSize: 11, color: C.textLight, fontWeight: 500 }}>Completion</span>
        <span style={{ fontFamily: "'Inter', sans-serif", fontSize: 12, fontWeight: 700, color: C.deep }}>{pct}%</span>
      </div>
...
export default function Attendance() {
  ...
  // ── Compute which names appear more than once ─────────────
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast(`Attendance submitted — ${summary.PRESENT} present, ${summary.ABSENT} absent`, "success", 4000);
    } catch {
  ...
  // ── Submit button (shared, used in header) ────────────────
  const SubmitButton = ({ size = "normal" }) => (
    <button
      disabled={saving || hasEmptyStatus}
      onClick={handleSave}
      style={{
        display: "flex", alignItems: "center", gap: 7,
        padding: size === "small" ? "8px 16px" : "10px 22px",
        ...
        boxShadow: saving || hasEmptyStatus ? "none" : `0 4px 12px ${C.deep}44`,
        opacity: saving || hasEmptyStatus ? 0.7 : 1,
        whiteSpace: "nowrap",
      }}
  ...
  return (
    <PageLayout>
      ...
      <div className="att-page" style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Inter', sans-serif", backgroundImage: `radial-gradient(circle at 15% 0%, ${C.mist}28 0%, transparent 50%)` }}>
        ...
        {/* ── Error Banner ── */}
        {initError && (
          <div className="fade-up" style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", borderRadius: 14, background: "#fffbeb", border: "1px solid #fcd34d", marginBottom: 18 }}>
            <AlertCircle size={15} style={{ color: "#d97706", flexShrink: 0 }} />
            <p style={{ color: "#92400e", fontSize: 13, fontWeight: 500, margin: 0, fontFamily: "'Inter', sans-serif" }}>{initError}</p>
          </div>
        )}

        {/* ── Stat Cards ── */}
        {students.length > 0 && (
          <div className="att-stat-grid fade-up" style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 14, marginBottom: 18 }}>
        ...
        {/* ── Filter / Control Bar ── */}
        <div className="fade-up" style={{ background: C.white, borderRadius: 18, border: `1.5px solid ${C.borderLight}`, boxShadow: "0 2px 16px rgba(56,73,89,0.06)", padding: "16px 18px", marginBottom: 18 }}>
          <div className="att-filter-bar" style={{ display: "flex", gap: 14, alignItems: "flex-end", flexWrap: "wrap" }}>

            {/* Class selector */}
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              <label style={{ fontFamily: "'Inter', sans-serif", fontSize: 11, fontWeight: 700, color: C.textLight, textTransform: "uppercase", letterSpacing: "0.07em" }}>
              ...
              <div style={{ position: "relative" }}>
                <select
                  ...
                  style={{ appearance: "none", WebkitAppearance: "none", border: `1.5px solid ${C.border}`, borderRadius: 12, padding: "9px 36px 9px 14px", fontSize: 13, fontWeight: 600, color: C.text, background: C.bg, outline: "none", cursor: "pointer", minWidth: 220, fontFamily: "'Inter', sans-serif" }}
                >
                ...
                <ChevronDown size={13} style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", color: C.textLight, pointerEvents: "none" }} />
              </div>
            </div>

            {/* Date picker */}
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              <label style={{ fontFamily: "'Inter', sans-serif", fontSize: 11, fontWeight: 700, color: C.textLight, textTransform: "uppercase", letterSpacing: "0.07em" }}>Date</label>
              <div style={{ position: "relative" }}>
                <CalendarDays size={13} style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: C.textLight, pointerEvents: "none" }} />
                <input type="date" value={date} onChange={(e) => setDate(e.target.value)} style={{ border: `1.5px solid ${C.border}`, borderRadius: 12, padding: "9px 14px 9px 32px", fontSize: 13, fontWeight: 600, color: C.text, background: C.bg, outline: "none", fontFamily: "'Inter', sans-serif" }} />
              </div>
            </div>

            {/* Right: progress + count + refresh */}
            <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 12px", borderRadius: 20, background: `${C.mist}55`, border: `1px solid ${C.borderLight}` }}>
                <Users size={12} color={C.textLight} />
                <span style={{ fontFamily: "'Inter', sans-serif", fontSize: 12, fontWeight: 700, color: C.deep }}>{students.length}</span>
                <span style={{ fontFamily: "'Inter', sans-serif", fontSize: 11, color: C.textLight }}>students</span>
              </div>
              <button
                onClick={() => loadStudents(selectedClassId, academicYearId, date)}
                style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: 12, border: `1.5px solid ${C.border}`, background: C.white, color: C.textLight, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}
        ...
        {loading ? (
          ...
          <div className="fade-up" style={{ background: C.white, borderRadius: 18, border: `1.5px solid ${C.borderLight}`, boxShadow: "0 2px 16px rgba(56,73,89,0.06)", overflow: "hidden" }}>
            ...
            <div style={{ padding: "14px 18px", background: `linear-gradient(90deg, ${C.bg}, ${C.white})`, borderBottom: `1.5px solid ${C.borderLight}`, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
              ...
              {/* Right: Mark All Present + Submit (both in header) */}
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>

                {/* Status chip */}
                {hasEmptyStatus ? (
                  <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 12px", borderRadius: 20, background: "#fffbeb", border: "1px solid #fcd34d" }}>
                    <AlertCircle size={11} color="#d97706" />
                    <span style={{ fontFamily: "'Inter', sans-serif", fontSize: 11, fontWeight: 700, color: "#92400e" }}>
                      {summary.UNMARKED} unmarked
                    </span>
                  </div>
                ) : (
                  <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 12px", borderRadius: 20, background: "#f0fdf4", border: "1px solid #86efac" }}>
                    <CheckCircle size={11} color="#22c55e" />
                    <span style={{ fontFamily: "'Inter', sans-serif", fontSize: 11, fontWeight: 700, color: "#15803d" }}>
                      All marked
                    </span>
                  </div>
                )}
                ...
                <button
                  ...
                  style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 12, background: `${C.sky}18`, border: `1px solid ${C.sky}44`, color: C.deep, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'Inter', sans-serif", whiteSpace: "nowrap" }}
                ...
                {/* Submit Attendance — PRIMARY CTA in header */}
                <SubmitButton size="normal" />
              </div>
            </div>

            {/* ── Search bar ── */}
            <div style={{ padding: "10px 18px", borderBottom: `1.5px solid ${C.borderLight}`, background: `${C.bg}88` }}>
              <div style={{ position: "relative", maxWidth: 360 }}>
                <Search size={13} style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: C.textLight, pointerEvents: "none" }} />
                <input
                  type="text"
                  placeholder="Search by name, father's name or roll number…"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  style={{ width: "100%", border: `1.5px solid ${C.border}`, borderRadius: 12, padding: "8px 36px 8px 34px", fontSize: 13, fontWeight: 500, color: C.text, background: C.white, outline: "none", fontFamily: "'Inter', sans-serif" }}
                  onFocus={(e) => (e.target.style.borderColor = C.sky)}
                  onBlur={(e)  => (e.target.style.borderColor = C.border)}
                />
                {searchQuery && (
                  <button onClick={() => setSearchQuery("")} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: C.textLight, padding: 0, display: "flex", alignItems: "center" }}>
                    <X size={13} />
                  </button>
                )}
              </div>
              {searchQuery && (
                <p style={{ fontSize: 11, color: C.textLight, margin: "5px 0 0 2px", fontFamily: "'Inter', sans-serif" }}>
              ...
              {filteredStudents.map((student, idx) => {
                ...
                return (
                  <div
                    ...
                    style={{ display: "flex", alignItems: "center", gap: 14, padding: "13px 18px", borderBottom: `1.5px solid ${C.borderLight}`, background: isUnmarked ? "#fffbeb" : C.white, transition: "background 0.15s", flexWrap: "wrap" }}
                  >
                    {/* Index */}
                    <span style={{ fontFamily: "'Inter', sans-serif", fontSize: 11, fontWeight: 600, color: C.textLight, width: 22, flexShrink: 0 }}>
                      {idx + 1}
                    </span>

                    {/* Avatar + Name (with father info for duplicates) */}
                   {/* Avatar + Name (with father info for duplicates) */}
                   {/* Avatar + Name */}
                    <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, minWidth: 140 }}>
                      <div style={{ width: 34, height: 34, borderRadius: "50%", background: `linear-gradient(135deg, ${C.sky}, ${C.deep})`, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 12, fontWeight: 700, flexShrink: 0, boxShadow: `0 2px 8px ${C.sky}44` }}>
                        {student.name?.charAt(0)?.toUpperCase() || "?"}
                      </div>
                      <div>
                        {/* Line 1: name + father full name */}
                        <div style={{ display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" }}>
                          ...
                          {isDuplicate && student.fatherName && (
                            <>
                              <span style={{ color: C.border, fontWeight: 400, fontSize: 13 }}>·</span>
                              <span style={{ fontSize: 12, fontWeight: 500, color: C.textLight, fontStyle: "italic", fontFamily: "'Inter', sans-serif" }}>
                                {student.fatherName}
                              </span>
                            </>
                          )}
                        </div>
                        {/* Line 2: roll + father surname badge */}
                        <div style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 2 }}>
                          <span style={{ fontSize: 10, color: C.textLight, fontFamily: "'Inter', sans-serif" }}>
                            Roll {student.rollNumber || "—"}
                          </span>
                          {isDuplicate && student.fatherName && (
                            <span style={{ fontWeight: 700, color: C.deep, fontSize: 10, background: `${C.sky}22`, padding: "1px 6px", borderRadius: 5, border: `1px solid ${C.sky}44`, letterSpacing: "0.03em", fontFamily: "'Inter', sans-serif" }}>
                    ...
                    <div style={{ display: "inline-flex", borderRadius: 12, border: `1.5px solid ${isPresent ? "#86efac" : isAbsent ? "#fca5a5" : C.border}`, overflow: "hidden", flexShrink: 0 }}>
                      <button
                        onClick={() => handleStatusChange(student.studentId, "PRESENT", student.name)}
                        style={{ display: "flex", alignItems: "center", gap: 5, padding: "8px 18px", border: "none", borderRight: `1.5px solid ${isPresent ? "#86efac" : C.borderLight}`, background: isPresent ? "rgba(34,197,94,0.12)" : C.white, color: isPresent ? "#15803d" : C.textLight, fontSize: 12, fontWeight: isPresent ? 800 : 600, cursor: "pointer", fontFamily: "'Inter', sans-serif", transition: "all 0.15s" }}
                      ...
                      >
                        <Check size={13} strokeWidth={isPresent ? 3 : 2} /> Present
                      </button>
                      <button
                        onClick={() => handleStatusChange(student.studentId, "ABSENT", student.name)}
                        style={{ display: "flex", alignItems: "center", gap: 5, padding: "8px 18px", border: "none", background: isAbsent ? "rgba(239,68,68,0.12)" : C.white, color: isAbsent ? "#b91c1c" : C.textLight, fontSize: 12, fontWeight: isAbsent ? 800 : 600, cursor: "pointer", fontFamily: "'Inter', sans-serif", transition: "all 0.15s" }}
                    ...
                    <input
                      ...
                      style={{ padding: "8px 14px", borderRadius: 12, border: "1.5px solid transparent", background: `${C.mist}33`, color: C.text, fontSize: 12, fontWeight: 500, outline: "none", fontFamily: "'Inter', sans-serif", transition: "border-color 0.15s, background 0.15s", width: 160, flexShrink: 0 }}
            ...
            {/* ── Card footer — status message only (no button here) ── */}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 18px", background: `linear-gradient(90deg, ${C.bg}, ${C.white})`, borderTop: `1.5px solid ${C.borderLight}`, flexWrap: "wrap", gap: 8 }}>
              ...
              {/* Repeat submit at bottom for long lists */}
              {students.length > 8 && <SubmitButton size="normal" />}
            </div>
          </div>
        ) : (
          /* Empty state */
