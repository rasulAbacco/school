// src/teacher/pages/chat/components/TeacherMessageView.jsx
...
function Toast({ message, type = "success", onClose }) {
  ...
  return (
    <div className={`fixed bottom-5 left-1/2 -translate-x-1/2 z-50 px-4 py-2.5 rounded-full text-sm font-semibold shadow-lg flex items-center gap-2 font-['DM_Sans'] ${type === "success" ? "bg-[#384959] text-white" : "bg-red-500 text-white"}`}>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-xs font-['DM_Sans']">
        ...
        <h3 className="text-center font-semibold text-[#384959] text-base mb-1">{title}</h3>
        <p className="text-center text-sm text-gray-500 mb-5">{body}</p>
        <div className="flex gap-3">
          <button className="flex-1 py-2.5 rounded-xl border border-gray-200 text-[#384959] text-sm font-semibold hover:bg-gray-50" onClick={onCancel}>Cancel</button>
...
const TeacherMessageView = ({ selectedChat, onBack }) => {
  const user = getUser();
  ...
  return (
    <>
      ...
      <div className="flex flex-col flex-1 bg-[#F4F8FC] h-[92dvh] min-w-0">
        {selectedChat ? (
          <>
            {/* Header */}
            <div className="bg-[#384959] flex-shrink-0">
              {/* Main header row */}
              <div className="p-3 sm:p-4 flex items-center gap-2.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    ...
                    <span className="text-xs text-[#BDDDFC] truncate">
              ...
              {showSearch && (
                <div className="px-3 pb-3 flex items-center gap-2">
                  <div className="flex-1 relative">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[#6A89A7]" size={14} />
                    <input
                      ref={searchInputRef}
                      className="w-full bg-white border border-blue-200 rounded-full pl-8 pr-3 py-2 text-sm text-[#384959] outline-none font-['DM_Sans']"
                ...
                const renderContent = () => {
                  ...
                  return parts.map((part, i) =>
                    part.toLowerCase() === q.toLowerCase()
                      ? <mark key={i} className="bg-yellow-300 text-[#384959] rounded px-0.5">{part}</mark>
                      : part
                  );
                };
                ...
                return (
                  ...
                  >
                    <div className={`flex items-end gap-2 ${isMe ? "flex-row-reverse" : "flex-row"}`}>
                      {!isMe && (
                        <div className="w-7 h-7 min-w-[28px] rounded-full flex items-center justify-center font-semibold text-xs bg-[#6A89A7] text-white flex-shrink-0">
                    ...
                    {/* Timestamp */}
                    <div className={`text-xs text-[#6A89A7] mt-0.5 ${isMe ? "text-right" : "text-left ml-9"}`}>
                      {msg.createdAt
            ...
            {/* Input */}
            <div className="p-3 border-t-[1.5px] border-[#BDDDFC] bg-white flex items-center gap-2 flex-shrink-0">
              ...
              <input
                className="flex-1 border-[1.5px] border-[#BDDDFC] rounded-full px-4 py-2.5 text-sm text-[#384959] outline-none bg-[#F4F8FC] font-['DM_Sans'] min-w-0"
                value={message}
              ...
              />
              <button
                className="bg-[#384959] text-white border-none rounded-full w-10 h-10 flex items-center justify-center cursor-pointer flex-shrink-0 hover:bg-[#6A89A7] transition-colors disabled:opacity-40"
                onClick={handleSend}
          ...
          <div className="flex-1 flex items-center justify-center flex-col gap-3 text-[#6A89A7] p-4">
            <div className="w-16 h-16 rounded-full bg-[#BDDDFC] flex items-center justify-center text-2xl">💬</div>
            <p className="text-sm text-[#6A89A7] font-medium text-center font-['DM_Sans']">
