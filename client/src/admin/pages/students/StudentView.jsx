...
import {
  ArrowLeft, Edit, Trash2, User, Mail, Phone, MapPin, BookOpen,
  Heart, Users, FileText, Calendar, Shield, Loader2, AlertCircle,
...
const Card = ({ title, icon: Icon, color = "blue", children }) => (
  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
    <div className={`flex items-center gap-3 px-5 py-4 border-b border-gray-100 bg-${color}-50/40`}>
    ...
    <PageLayout>
      <div className="flex items-center justify-center h-96">
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <p className="font-semibold text-gray-700">{error || "Student not found"}</p>
          <button onClick={() => navigate("/students")} className="text-sm text-blue-600 hover:underline">← Back to list</button>
        </div>
      </div>
    </PageLayout>
  );
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
            <div className="w-20 h-20 rounded-2xl overflow-hidden bg-gradient-to-br from-blue-400 to-blue-700 flex items-center justify-center text-white text-2xl font-bold shadow-md shrink-0">
              {pi?.profileImage
                ? <img src={pi.profileImage} alt={fullName} className="w-full h-full object-cover" />
                : `${pi?.firstName?.[0] || ""}${pi?.lastName?.[0] || ""}`.toUpperCase() || <User size={32} />}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-3 mb-1">
                <h1 className="text-2xl font-bold text-gray-900">{fullName}</h1>
                {pi?.status && (
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold border ${statusColor(pi.status)}`}>
              ...
              <div className="flex flex-wrap items-center gap-4 mt-3 text-sm text-gray-500">
                {pi?.grade     && <span className="flex items-center gap-1"><BookOpen size={13} />{pi.grade}</span>}
                {pi?.className && <span className="flex items-center gap-1"><Shield size={13} />{pi.className}</span>}
                {pi?.admissionDate && (
                  <span className="flex items-center gap-1">
                    <Calendar size={13} />
                    Admitted {new Date(pi.admissionDate).toLocaleDateString("en-IN", { day:"2-digit", month:"short", year:"numeric" })}
                  </span>
                )}
              </div>
            </div>
            <div className="shrink-0 text-right">
              ...
              <p className="font-mono text-xs text-gray-600 bg-gray-100 px-2.5 py-1.5 rounded-lg">{student.id.slice(0, 8).toUpperCase()}</p>
              <p className="text-[10px] text-gray-400 mt-2">
                Registered {new Date(student.createdAt).toLocaleDateString("en-IN", { day:"2-digit", month:"short", year:"numeric" })}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="space-y-5">
            <Card title="Personal Information" icon={User} color="blue">
              <InfoRow icon={Calendar} label="Date of Birth" value={pi?.dateOfBirth ? new Date(pi.dateOfBirth).toLocaleDateString("en-IN",{day:"2-digit",month:"long",year:"numeric"}) : null} />
              <InfoRow icon={User}     label="Gender"        value={pi?.gender ? pi.gender.charAt(0) + pi.gender.slice(1).toLowerCase() : null} />
              <InfoRow icon={Heart}    label="Blood Group"   value={pi?.bloodGroup?.replace("_PLUS","+").replace("_MINUS","-")} />
            </Card>

            <Card title="Health Information" icon={Heart} color="pink">
              <InfoRow icon={AlertCircle} label="Medical Conditions" value={pi?.medicalConditions || "None recorded"} />
              <InfoRow icon={AlertCircle} label="Allergies"          value={pi?.allergies        || "None recorded"} />
            </Card>
          </div>
          ...
          <div className="space-y-5">
            ...
            <Card title="Academic Information" icon={BookOpen} color="green">
              ...
              <InfoRow icon={Calendar} label="Admission Date" value={pi?.admissionDate ? new Date(pi.admissionDate).toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"}) : null} />
            </Card>
          </div>
          ...
          <div className="space-y-5">
            <Card title="Parent / Guardian" icon={Users} color="orange">
              ...
              <InfoRow icon={Phone} label="Emergency Contact" value={pi?.emergencyContact} />
            </Card>

            {/* Documents */}
            <Card title={`Documents (${docs.length})`} icon={FileText} color="indigo">
              {docs.length === 0 ? (
                <div className="flex flex-col items-center py-6 text-center">
                  <FileText size={28} className="text-gray-200 mb-2" />
                  <p className="text-sm text-gray-400">No documents uploaded</p>
                  <button onClick={() => navigate(`/students/${id}/edit`)}
                    className="mt-3 text-xs text-blue-600 hover:underline font-medium">Upload now</button>
                </div>
              ) : (
                <div className="space-y-2">
                  {docs.map((doc) => {
                    const isImg = doc.mimeType?.startsWith("image/");
                    const label = doc.documentName === "CUSTOM"
                      ? doc.customLabel || "Custom"
                      : doc.documentName.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
                    return (
                      <div key={doc.id} className="flex items-center gap-3 p-2.5 bg-gray-50 rounded-xl">
                        <div className="w-8 h-8 rounded-lg bg-white border border-gray-200 flex items-center justify-center shrink-0">
                          {isImg ? <ImageIcon size={14} className="text-blue-500" /> : <FileIcon size={14} className="text-orange-500" />}
                        </div>
                        <div className="flex-1 min-w-0">
